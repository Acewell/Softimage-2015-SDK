...
/*! \class TimerEvent xsi_timerevent.h
	...
	the delay time at which a timer elapses. Timers in Softimage are independent of the timeline
	clock and can be used to produce recurrent work accurately. For example, you can implement an
	autosave mechanism with a TimerEvent.

	The TimerEvent class derives from EventInfo and can be managed like a regular Softimage event.
	Use PluginRegistrar::RegisterTimerEvent to register them and Application::GetEventInfos to access
	them. Timer events can be suspended with muting and resumed by un-muting (see EventInfo::PutMute).
	You can reset a timer with TimerEvent.Reset.
...
public:
	...
	*/
	...
	\return CStatus::Fail Failure. This can occur when in_interval and in_delay
	are both 0.
