...
/*! \class Shader xsi_shader.h
	...
	a phong, for example \c "incandescence", \c "reflectivity", \c "trans_glossy".
	...
	ProjectItem::GetParameters.  They are also documented in the
	\xp intro %XSI %Parameter %Reference \endxp
...
public:
	...
	*/
	...
	\param in_obj The shader object to be copied into this object.
	\return The reinitialized shader object.
	*/
	Shader& operator=(const Shader& in_obj);


	/*! Assigns a CRef to this shader object. The shader object is cleared if the CRef
	is not a reference to an object that supports the %Shader class.
	...
	*/
	...
	\note Since v7.0, this function also returns the ProgID of shader compounds with the following syntax:<br><br>
		\c XSIRTCompound.$(ShaderCompoundName).$(MajorVersion).$(MinorVersion)
		<br><br>If the shader compound has not been exported and cannot be created it returns only \c XSIRTCOMPOUND.
	\return The string representing the ProgID.
	\sa Parameter::ConnectFromProgID,
	*/
	...
	*/
	...
	\retval out_red The Red component value, typically a number in the range \c 0.0 to \c 1.0.
	\retval out_green The Green component value, typically a number in the range \c 0.0 to \c 1.0.
	\retval out_blue The Blue component value, typically a number in the range \c 0.0 to \c 1.0.
	...
	\param in_time Time (in frames) at which to get the value. By default, GetColorParameterValue gets the
		value at the current frame (represented by the value \c DBL_MAX).
	*/
	...
	\param in_name The scripting name of the color parameter, for example \c "ambient".
	\param in_red The Red component value, typically a number in the range \c 0 to \c 1.0.
	\param in_green The Green component value, typically a number in the range \c 0 to \c 1.0.
	\param in_blue The Blue component value, typically a number in the range \c 0 to \c 1.0.
	...
	\param in_time Time (in frames) at which to set the color values. By default, PutColorParameterValue sets the
		value at the current frame (which is represented by the value \c DBL_MAX).
	*/
	...
	\param in_bAfter True to insert the layer after the reference layer (\c in_refLayer), and False to insert the layer
	...
	*/
	...
	\param in_layer TextureLayer to remove from this container.
	\return CStatus::OK if successful
	...
	*/
	...
	\param in_preset A preset name or the name of a \xt cus_prop self-installed %CustomProperty\endxt .
	\param in_bBranch Should always be false.
	\param in_name Optional name for the new property.  If no name is specified, the name is based on \c in_preset.
	...
	*/
	...
	/*! Returns the input type of a parameter. Before you connect a Shader to the parameter, use Parameter::GetCapabilities
	to make sure that the parameter supports the ::siTexturable capability.
	...
	\param in_scriptname  The script name of a parameter.
	\return The shader input type (one of the ::siShaderParameterType values). If the parameter cannot be connected to
		a shader, ::siUnknownParameterType is returned.
	\since 5.0
	*/
	...
	\param in_portScriptName The script name of the port parameter.
	\return CString containing the shader port's display name, or "" if no name was set.
	\since 6.01
	*/
	...
	\param in_portScriptName The script name of the port parameter.
	\param in_displayName The shader port's new display name.  If the string is empty, it will be reset to its original value.
	\return CStatus::OK if the display name was successfully set; CStatus::Fail otherwise.
	\since 6.01
	*/
	...
	\note As of v7.0, shaders can live without being connected to a root, so this function now can return an empty CRef.
	\since 5.0
	*/
	CRef GetRoot(void) const;

	/*! Returns the shader type that identifies if the shader is a normal shader or a shader compound.
	\return The shader type or ::siShaderUnknown if the call fails.
	\since 7.0
	*/
	siShaderType GetShaderType() const;

	/*!	Returns a ::siShaderParameterType value describing whether the parameter is an input or output and whether or not the
	shader is a compound.
	\tip On single output shaders, you can use an empty scriptname Parameter::GetScriptName to get the parameter type of the output.
	\note Before you connect a shader to the parameter, use Parameter::GetCapabilities to make sure that the parameter supports
		the ::siTexturable capability (see the ::siCapabilities enum).
	\param in_paramscriptname The script name of an input or output parameter or the empty string to query for the output of a
		single output shader.
	\param out_input Returns true for an input parameter and false for an output parameter.
	\return The shader parameter type.  If the parameter cannot be connected to a shader, ::siUnknownParameterType is returned.
	\since 7.0
	*/
	siShaderParameterType GetShaderParameterType( const CString& in_paramscriptname, bool& out_input ) const;

	/*! Returns the shaders nested under a compound shader. You can test to see whether a shader is a compound by calling the
	Shader::GetShaderParameterType function (compounds return ::siShaderCompound).
	\return Array of references to the nested shader objects.
	\sa Shader::GetShaderParameterType, ::siShaderParameterType
	\since 7.0
	*/
	CRefArray	GetAllShaders() const;

	/*! Returns a reference to the shader container under which this shader is nested. %Shader containers can be
	shader compounds, materials, cameras, or lights.
	\return Reference to the shader container.
	\sa Material, Camera, Light
	\since 7.0
	*/
	...
	\tip On single output shaders, you can use an empty scriptname Parameter::GetScriptName to get the parameter type of the output.
	\param in_paramscriptname The script name of an output parameter or the empty string to query for the output of a single output shader.
	\return Array of references to the target shader input Parameter objects.
	\since 7.0
	*/
	CRefArray GetShaderParameterTargets( const CString& in_paramscriptname ) const;

	/*! Returns the image clips nested under a compound shader. You can test to see whether a shader is a compound by calling the
	Shader::GetShaderParameterType function (compounds return ::siShaderCompound).
	\return Array of references to the nested ImageClip2 objects.
	\sa Shader::GetShaderParameterType, ::siShaderParameterType, Light::GetAllImageClips, Camera::GetAllImageClips,
		Material:GetAllImageClips, Shader::GetAllImageClips, Override::GetAllImageClips
	\since 7.0
	*/
	CRefArray	GetAllImageClips() const;

	/*! Returns the shader definition associated with the shader.
	\return A reference to the ShaderDef object
	\since 9.0 (2011)
	*/
