...
/*! \class Operator xsi_operator.h
	...
	or picked object. For example, when the \xp twistop Twist operator \endxp is applied to a selected
	object the operator has ports that read from the object's local KinematicState and Geometry and
	write to the result object's geometry.
...
public:
	...
	*/
	...
	\param  in_Force True to disconnect the object even if the port is not optional.
