...
public:
	...
	*/
	...
	\return CStatus::AccessDenied Access denied if the ActionSource is used by a locked animation layer (see <command idref="IsAnimationLayerLocked"/>).
	...
	*/
	...
	\tip In the mixer, you can see this name ("Item Name") by opening the property page
		of the source (for example, %Mixer &gt; %Sources &gt; %Shape &gt; Point_ClusterClip_source.
		In the case of a shape source, you need to access Clip::GetMappedItems when you need a current
		cluster name changed by a designer, because this function returns the original cluster name
