...
/*! \class Source xsi_source.h
	...
	\sa Clip::GetSource, ActionSource, Model::GetSources, \xl AddImageSource, \xl ImportAudio
	\since 4.0
...
public:
	...
	*/
	...
	\note Only \xt CustomPropertyPresets CustomProperty Presets\endxt, \xt UserDataBlobPresets UserDataBlob Presets\endxt,
		and \xt UserDataMapPresets UserDataMap Presets\endxt are valid for sources.
	\param in_preset Either the name of a \xt PropPreset Property Preset\endxt or a string with the filename
		or full path to a Preset file. The type of property that is created is determined by this argument. For example,
		\c "CustomProperty" creates an empty CustomProperty and \c "UserDataBlob" creates a UserDataBlob.
	\param in_bBranch False is the only supported value
	\param in_name Represents the name of the new property (see SIObject::Name). If not specified the object is named
		based on the value specified for \c in_preset.
