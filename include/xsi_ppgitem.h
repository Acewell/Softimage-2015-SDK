...
/*! \class PPGItem xsi_ppgitem.h
	...
	Each PPGItem exposes a series of \a attributes that can be used to tweak its appearance. Some of the most
	...
	\sa ProjectItem::GetPPGLayout, PPGLayout::AddItem, PPGLayout::GetItem,
		\xt cus_ppg_PropertyPageControls %Property Page Controls \endxt
	\since 4.0
	...
	\code
		...
		// You can use the string directly, or the typdef siUIInitialDir (one of the siPPGItemAttribute string constants)
...
public:
	...
	*/
	...
	\return The control's label text
	...
	*/
	...
	\param in_Label The control's label text
	\sa PPGItem::GetLabel, PPGItem::GetName, Parameter::GetScriptName, PPGLayout::AddItem,
		\xt cus_ppg_Labels Labels \endxt
	*/
	CStatus PutLabel( const CString& in_Label ) ;

	/*! Returns the control type to display on the %Property Page.
	\return  String containing one of the \link siPPGControlType \endlink strings.
	\sa PPGItem::PutType, \xt cus_ppg_PropertyPageControls %Property Page Controls \endxt
	*/
	CString GetType() const ;

	/*! Sets the control type to display on the %Property Page.  This property is optional: when not
	set, Softimage picks a control based on the type of the parameter (see Parameter::GetValueType).
	For example, a ::siDouble uses a ::siControlNumber.

	\param in_Type CString containing one of the \link siPPGControlType \endlink string constants
	\sa PPGLayout::AddItem, \xt cus_ppg_PropertyPageControls %Property Page Controls \endxt
	*/
	CStatus PutType( const CString& in_Type ) ;

	/*! Returns the list of items and values associated with a control. This property is used by
	controls which display items from a fixed list of choices, for example ::siControlCombo,
	::siControlCheck, ::siControlRadio, and ::siControlIconList.

	\return  CValueArray containing the label/value pairs
	\sa	PPGLayout::AddEnumControl, PPGItem::PutUIItems, \xt cus_ppg_EnumControls Enum Controls \endxt
	*/
	CValueArray GetUIItems() const ;

	/*! Sets the list of items and values associated with a control as a 1-dimensional array of
	Label/Value pairs.  This property is used by controls which display items from a fixed list
	of choices, for example ::siControlCombo, ::siControlCheck, ::siControlRadio, and ::siControlIconList.
	Each label is a string and the value is a CValue whose type should match the type of the associated
	parameter (see Parameter::GetValueType).

	\param in_items CValueArray containing a label and value for each item
	\sa	PPGLayout::AddEnumControl, PPGItem::GetUIItems, \xt cus_ppg_EnumControls Enum Controls \endxt
	*/
	CStatus PutUIItems( const CValueArray & in_items ) ;

	/*! Returns the WidthPercentage.
	\return Width of the control as a percentage of the total width of the property page
	\sa PPGItem::PutWidthPercentage, \xt cus_ppg_SettingControlAttributes Setting Control Attributes \endxt
	*/
	LONG GetWidthPercentage() const ;

	/*! Returns the WidthPercentage. This property is only applicable if the group is part of a row.
	It gives a hint to Softimage about how much of the horizontal space should be taken by this item.
	If not specified, Softimage uses its own heuristic to lay out the row.

	\param in_percent Width of the control as a percentage of the total width of the property page
	\sa PPGLayout::AddGroup, PPGItem::GetWidthPercentage, \xt cus_ppg_SettingControlAttributes Setting Control Attributes \endxt
	*/
	CStatus PutWidthPercentage(LONG in_percent) ;

	/*! Returns the width of the label of a control, in terms of its percentage of the control's total width.
	\return Width of the control's label
	\sa	PPGItem::GetLabelMinPixels, PPGItem::GetWidthPercentage, PPGItem::GetAttribute,
		\xt cus_ppg_SettingControlAttributes Setting Control Attributes \endxt
	*/
	LONG GetLabelPercentage() const ;

	/*! Sets the width of the label of a control, in terms of its percentage of the control's total width.
	When not set, Softimage will use its own heuristic, but this can be useful to give a hint when tweaking
	the layout.  For example this can be useful to force a \c LONG label onto multiple lines rather than
	squish the controls.

	To completely hide a label use the ::siUINoLabel or ::siValueOnly UI Attribute (see \link siPPGItemAttribute \endlink).

	This function should always be used in conjunction with PPGItem::PutLabelMinPixels.

	\param in_percent Width of the control's label
	\sa PPGItem::GetLabelPercentage, PPGItem::PutLabelMinPixels, PPGItem::PutWidthPercentage,
		PPGItem::PutAttribute, \xt cus_ppg_SettingControlAttributes Setting Control Attributes \endxt
	*/
	CStatus PutLabelPercentage(LONG in_percent) ;

	/*! Returns the minimum width, in pixels, to use when drawing a label.
	\return Minimum width for the label
	\sa PPGItem::GetLabelPercentage, PPGItem::PutLabelMinPixels, PPGItem::GetAttribute,
		\xt cus_ppg_SettingControlAttributes Setting Control Attributes \endxt
	*/
	LONG GetLabelMinPixels() const ;

	/*! Sets the minimum width, in pixels, to use when drawing a label. A label will get split into
	multiple lines when there is not enough space on a single line.

	This function should always be used in conjunction with PPGItem::PutLabelPercentage

	\param in_pixels Minimum width for the label
	...
	*/
	...
	/*! Sets an attribute on the control.  Each control has a collection of attributes which can be
	used to tweak its appearance or to provide it with additional states.  Some of these attributes
	are general and apply to almost any control, for example ::siUIAlignCenter.  Others are specific
	to the control, for example ::siUINoSlider (which only applies to numeric controls).

	If the attribute argument is not a recognized attribute this method fails.  It is possible to
	remove an attribute by calling this method with an empty CValue as the AttributeValue argument.

	For convenience, some of the most common attributes have been exposed directly as properties,
	for example PPGItem::PutLabel and PPGItem::PutUIItems.

	\param in_UIAttribute CString specifying which attribute to set.  The \link siPPGItemAttribute \endlink
		string constant provides a complete list of supported attributes.
	...
	*/
	...
	/*! Returns an attribute on the control.  Each control has a collection of attributes which can be
	used to tweak its appearance or to provide it with additional states.  Some of these attributes are
	general and apply to almost any control, for example ::siUIAlignCenter.  Others are specific to the
	control, for example ::siUINoSlider (which only applies to numeric controls).

	If the attribute argument is not a recognized attribute the call to read it fails.  Most of the
	attributes are optional and Softimage returns an empty CValue if the attribute has not been set.

	For convenience, some of the most common attributes have been exposed directly as properties, for
	example PPGItem::GetLabel and PPGItem::GetUIItems.

	\param in_UIAttribute CString specifying which attribute to get.  The \link siPPGItemAttribute \endlink
		string constant provides a complete list of supported attributes.
