...
/*! \class ViewContext xsi_viewcontext.h
	\brief The ViewContext object serves 2 functions for Custom Displays:
		...
		\li Provides information about a notification sent by Softimage
...
public:
	...
	*/
	...
	/*! Returns the current view Metadata associated with the Context.
	Metadata is string data that can be used during the early initialization of a Custom Display Host.
	It also provides another way to store more data should UserData already be filled with a value.
	The "metadata" view attribute can be set via SetAttributeValue() on the view before the "targetcontent"
	view attribute is set, allowing the Custom Display Host to recognize the "metadata" view attribute early on.
	\since 11.0 (2013)
	\return a string containing user-specified Metadata.
	*/
	...
	*/
	...
	/*! Allows you to store a CValue inside the context object. This is called \a User \a Data because
	Softimage does not care about the content or meaning of the data.  This information is not persisted.
	A common use for this feature is to store a pointer to a newly allocated C++ object during an \c Init
	callback, and then to use Context::GetUserData to retrieve the pointer when other callbacks are made.
	(In this scenario the object should be destroyed in the \xt cb_Plugin_Term Term \endxt callback).
	...
	*/
	...
	\note This is only relevant on Linux.  This is needed so that Softimage can parent the Custom Display properly
		inside the View that hosts the Custom Display.
	...
	/*! Returns the top level widget from Softimage. This is necessary in order to create your custom display as a
