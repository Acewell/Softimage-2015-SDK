...
/*! \class Command xsi_command.h
	\brief Represents a Softimage command (either built-in or custom).  This object encapsulates information on a command
	such as the name, category, arguments, where it is implemented etc.

	Custom commands behave just like built-in Softimage commands--they are logged in the history window and can be exposed
	to scripting.

	You can access all the built-in and custom commands installed in the system with Application::GetCommands.

	\note Make sure the name you use as a key to access a command in Softimage is the string returned by SIObject::GetName
	...
	You can also place commands in toolbars and menus. There are two ways to place custom commands in Softimage menus.  The
	first is through the command category (see the ::siCommandCategory enum) and the second, more powerful approach, is through
	the Menu API.
	...
	Softimage supports three possible ways to define a custom command: the embedded command, the plug-in based command and the
	v1.5 command.  They are all based on the same Command object API but they have some subtle differences. The plug-in based
	command is the most convenient for commands implemented with the C++ API.  For information about the other approaches please
	see the \xl %Command object documentation in the \a Scripting \a Reference guide.
	...
	\note For more information see \xt cus_cmds Custom Commands \endxt.
...
public:
	...
	*/
	...
	\param in_Str The new scripting name to set.
	...
	*/
	...
	implements the command. In other words, the Handler is the name of the callback function that Softimage calls each time the
	command is invoked.
	...
	*/
	...
	as a script.
	\deprecated 9.5 (2011) This method is obsolete and must not be used. 
	*/
	...
	\li \c CPP (using the Softimage C++ API)
	\li \c C# (C# using the Softimage Object %Model via .NET)
	\li \c Python (Python scripting language using the Softimage Object %Model)
	\li \c JScript (JScript using the Softimage Object %Model)
	\li \c VBScript (VBScript using the Softimage Object %Model)
	...
	scripting language, C#, or the C++ API.
	\since 4.0
	*/
	CString GetLanguage() ;
	...
	*/
	...
	/*! Returns true if the custom command returns a value.  If you call a command that does not return a value then Softimage always passes
	...
	*/
	...
	/*! Optionally sets a descriptive string for the command. The description is shown in the Softimage user interface (for example on the
	\a Customize \a Toolbar dialog).
	\since 4.0
	*/
	...
	*/
	...
	/*! For advanced users.  Returns the GUID that Softimage uses internally to distinguish between different custom commands.  This can
	be used with the \xl CreateToolbarButton command.
	\since 4.0
	*/
	CString GetUID() ;
	...
	*/
	...
	/*! Distinguishes between custom commands and the commands that ship as part of the Softimage application (called either \a native or
	\a built-in commands).
	\since 4.0
	*/
	bool IsBuiltin() ;
	...
	\param out_return The value returned by the underlying command. If the command has no return value explicitly defined,
		Execute returns all output arguments in a CValueArray object. However, if the command defines a return value, you
		cannot extract any output arguments from it. This is because the command is not returning an output argument array,
		but a specific value. You can check the Return Value section in the reference documentation to see whether it uses
		an explicit return value and what that value is.
	...
	*/
	...
	\param in_whichflag	Flag to set.
	\param in_newvalue	True to enable the flag (default); false to disable it.
	\since 4.0
	*/
...
class SICPPSDKDECL CommandArray : public CBase
{
	...
	*/
	...
	\param in_keywords Keywords to match
	\since 4.0
	*/
	...
	\param in_keyword Keyword to match
	\since 4.0
	*/
