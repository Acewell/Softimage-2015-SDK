...
/*!
	\file BaseShape.h
	\brief Defines the CSLBaseShape class, as well as the SLIndexedVector2D, SLIndexedVector3D, and SLIndexedColor structures.
...
/*! \brief Associates an index to a CSIBCVector2D object.
*/
...
/*! \brief Associates an index to a CSIBCVector3D object.
*/
...
/*! \brief Associates an index to a CSIBCColorf object.
*/
...
/*! \brief Describes the shape of a mesh, containing information about the position coordinates, normal coordinates
and color values of a mesh.

There are two possible types of shape: ordered and indexed. The set of functions
that can be used depends on the type of shape that is described by each class instance.

For example, the CreateNormals() function will work on an ordered shape, but won't on an indexed shape. On the
other hand, the CreateIndexedNormals() function will work on an indexed shape, but won't on an ordered shape.
However, the DestroyNormals() and GetNormalCount() functions will work with both types (ordered and indexed).

\eg This example demonstrates how CSLBaseShape is used
\code
