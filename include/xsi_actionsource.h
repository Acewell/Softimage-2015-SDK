...
/*! \class ActionSource xsi_actionsource.h
	...
	\xl StoreAction), and by plotting (\xl PlotToActions and \xl PlotAndApplyActions).
	...
	\xl LoadActionPreset, \xl SIStoreAction, \xl StoreAction, \xl PlotToActions,
	\xl PlotAndApplyActions, \xl CreateEnvironmentCache, \xl StoreEnvironmentCacheToMixer
...
public:
	...
	*/
	...
	\param in_dStartFrame	The frame to start offloading the fcurve keys.
	\param in_dEndFrame		The frame to end offloading the fcurve keys.
	\param in_bRemember		True to append the range of frames to the \c OffloadedRanges parameter of the action, so when
		the scene is reloaded the partial offload will be processed.
	\param in_bRemember		False to offload the range of frames only temporarily.
	\return CStatus::OK success
	\return CStatus::Fail is returned in case of failure
	\sa \xl %ActionSource.SourceItems
	\since 4.0
