...
/*! \class ShapeKey xsi_shapekey.h
	...
	In the context of a ShapeKey, values returned by ClusterProperty::GetValues are automatically
	converted to object-based referential, which is not the case for ClusterProperty::GetElements.
	...
	\warning The KeyType parameter uses a different set of values from the ::siShapeReferenceMode
		enum. For more information, see \xt ShapeAnimation Shape Animation \endxt
	...
	\sa ClusterProperty::GetValues, AnimationSourceItem::GetSource, ActionSource::GetItems, ShapeClip, ActionSource,
...
public:
	...
	*/
	...
	/*! Sets the frequency of the stored keys in the ShapeKey

	\param in_dFrequency frequency of keys in frames per second

	\since 13.0 (2015)
	*/
	CStatus PutFrequency(double in_dFrequency);

	/*! Gets the frequency of the stored keys

	\return Gets the frequency in frames per second

	\since 13.0 (2015)
	*/
	double GetFrequency() const;

	/*! Sets the number of keys inside the ShapeKey

	\param in_nbKeys number of keys to set

	\since 13.0 (2015)
	*/
	CStatus ReserveNumKeys(LONG in_nbKeys);

	/*! Gets the number of keys inside the ShapeKey

	\since 13.0 (2015)
	*/
	LONG GetNumKeys() const;

	/*! Sets the position values in a specified key index

	\param in_pElements indices of the elements to set
	\param in_pValues position values stored in (x, y, z) order
	\param in_nElements number of elements to set (the number of floats in in_pValues divided by 3)
	\param in_lKeyIndex the key to set the values in

	\since 13.0 (2015)
	*/
	CStatus SetValuesByKeyIndex(const LONG*	in_pElements, const float* in_pValues, LONG in_nElements, LONG in_lKeyIndex);

	/*! Gets the position values from a specified key index

	\param out_array output array that will contain the positions
	\param in_lKeyIndex index of the key to get values from

	\since 13.0 (2015)
	*/
	CStatus GetValuesByKeyIndex(CFloatArray& out_array, LONG in_lKeyIndex) const;
