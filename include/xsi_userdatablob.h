...
public:
	...
	*/
	...
	/*! Provides access to the data inside the UserDataBlob. The \c out_buffer will be set to point
	to NULL if there is no user data on the object.

	For performance purposes, the returned pointer points directly to internal Softimage data.  The
	caller does not need to free this memory and should treat this data as read-only and potentially
	transitive.  This pointer should not be held by the caller, and, if necessary, the data should be
	copied locally.
	...
	*/
	...
	Softimage will copy the provided buffer so that the caller does not need to maintain the provided
	data in memory.

	\tip Passing \c 0 as the size is equivalent to calling UserDataBlob::Clear.
	...
	*/
	...
	testing is UserDataBlob::Size returns \c 0.
	*/
	bool IsEmpty() const ;
