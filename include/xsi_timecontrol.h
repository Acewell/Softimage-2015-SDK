...
/*! \class TimeControl xsi_timecontrol.h
	...
	Access to %TimeControl elements is read-only (eg, TimeControl::GetScale, TimeControl::GetClipIn,
	TimeControl::GetExtrapolationBeforeValue, etc.); however, you can still use the pointer to the
	%TimeControl object to write to its parameters (since the %TimeControl object is a specialized
	kind of Property object).
