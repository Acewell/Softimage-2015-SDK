...
/*! \class Plugin xsi_pluginitem.h
	\brief A Plugin object holds read-only information on a particular plug-in file registered in Softimage.
	...
	\eg Shows how to access information about the plug-ins that are currently registered in Softimage
	...
	\code
		...
		for (LONG i=0; i<pluginArray.GetCount(); i++)
		{
			...
			app.LogMessage( L"help button: " + plugin.GetHelpButton() );
			...
			for (LONG j=0; j<itemArray.GetCount(); j++)
			{
				...
				app.LogMessage( L"    item help button: " + pluginItem.GetHelpButton() );
...
class SICPPSDKDECL Plugin : public SIObject
{
	...
	*/
	...
	/*! Stores a CValue inside the plug-in object. Softimage does not care about the content
	or meaning of the data.  Once created, the data remains active as long as Softimage is
	running or until you manually unload the plug-in.

	You can use this to store a value to be accessed locally in the plug-in implementation
	or accessed globally by other plug-ins. The user data is not persisted.

	\warning In the case where a pointer is stored, the object should be destroyed
		during the \xt cb_Plugin_Unload UnloadPlugin \endxt callback.

	\param in_val User data to set.
	\return CStatus::OK success
	\return CStatus::Fail failure.
	\sa Application::UnloadPlugin
	\since 6.01
	 */
	...
	*/
	...
	/*! Returns the help button file replacing the factory "?" button
	in the property pages of the plug-in items of this plug-in.
