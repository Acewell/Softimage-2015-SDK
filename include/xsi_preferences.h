...
/*! \class Preferences xsi_preferences.h
	\brief The Preferences object can be used to manage preferences in Softimage.
	...
	viewed with the File->%Preferences dialog. By default, no preferences file is created until you change
	the value of a preference, thereby changing the location of preference to \c user. When Softimage starts
	it reads all \c .xsipref files located under the \c /Data/Preferences directory of the Factory, Workgroup
	and User locations (in that order), so if the value exists in both the factory and the user locations, the
	value in the user file will be used.
...
public:
	...
	*/
	...
	/*! Imports a set of preferences into Softimage. All values are saved in the \c .xsipref under the
	user location. The file can contain preferences from any category. The file must be in the \c .xsipref
	file format.
	...
	*/
	...
	/*! Exports one or more categories of preferences into a \c .xsipref file.
