...
/*! \class Filter xsi_filter.h
	\brief The %Filter object represents an instance of a Softimage filter. Filters are used by Softimage interactive
	tools and OM objects for validating the use of a set of objects in a given context. Softimage already defines a set
	of native filters available to users and can be extended with custom filter objects.

	Custom filters are defined with a plug-in which can be either compiled or scripted as described in
	\xt cus_addons %Distributing %Customized %Items \endxt for the detailed workflow.
	...
	\eg Demonstrates how to work with filters in Softimage
