...
/*! \class PPGLayout xsi_ppglayout.h
	...
	(\c PPG is a common acronym for %Property Page). The layout makes it possible to group objects together, use
	...
	A PPGLayout is modeled in Softimage as a linear list of PPGItem objects.  Each PPGItem represents either a
	parameter in the inspected object, or a special UI item like a Tab or Button.  In most cases the layout does
	not specify hard-coded pixel positions or dimensions for the PPGItems.  Instead, Softimage automatically draws
	the controls with a sensible layout.  This allows the presentation to change in a pleasing manner when the user
	resizes the property page.  However there is some support for providing hints, for example PPGItem::PutWidthPercentage,
	or even explicit sizes via PPGItem::PutAttribute.
	...
	Practically every built-in Softimage object has its own layout.  However, if there is no specific layout, Softimage
	displays a default layout that lists all the parameters in a simple fashion.

	Layouts are remembered in memory while Softimage is running.  The layouts for built-in objects are reloaded from
	SPDL files when Softimage restarts.  The layout for a CustomProperty object implemented as a PluginItem is defined
	by implementing the \xt cb_Property_DefineLayout DefineLayout \endxt callback.
	...
	\note If the layout is changed from within the %PPGEvent callback it is necessary to set the \c Refresh attribute
	...
	\sa PPGItem, ProjectItem::GetPPGLayout, PPGEventContext, PluginRegistrar::RegisterProperty,
		\xt cus_ppg_Creating_Dialogs Building Dialogs \endxt, \xm PSetUIDemo/netview_PSetUIDemo PSetUIDemo example \endxm
	\since 4.0
...
public:
	...
	*/
	...
	Often a call to this function is followed by one or more calls to PPGItem::PutAttribute to provide additional
	information about the appearance of the item.
	...
	\param in_Type Specify the control to draw.  A complete list of possible controls is listed in the
		\link siPPGControlType \endlink string constants. For example this argument can be used to specify
		that a parameter should be displayed as a combo box. (See PPGItem::GetType).  If not specified,
		Softimage uses a simple heuristic for determining what control to use, based on the
		Parameter::GetValueType. For example, strings have an edit control, numeric types like float and
		integer use a slider, and boolean parameters use a checkbox.

	\return The newly created PPGItem.
	\sa PPGItem::GetLabel, Parameter::GetScriptName, \xt cus_ppg_PropertyPageControls %Property Page Controls \endxt
	*/
	...
	\sa PPGLayout::AddItem, \xt cus_ppg_MultilineText Multi-line Text controls \endxt
	...
	*/
	...
	\param in_UIType Specify the control to draw.  A complete list of possible controls is provided in the \link siPPGControlType \endlink
		string constants, but in the case of this function only certain controls, such as ::siControlCombo, ::siControlRadio,
		::siControlCheck, and ::siControlIconList are supported.

	\sa PPGLayout::AddItem, PPGItem::GetUIItems, \xt cus_ppg_EnumControls Enum Controls \endxt
	*/
	...
	for example \c MyR, \c MyG, \c MyB, and \c MyA. The naming scheme is not important but the parameters must be consecutive.  This
	parameters should have a UI Range of \c 0 to \c 1 because the values are stored in normalized format.  However you can have a larger
	...
	method and specifying the name of the first parameter, for example \c MyR.  To read the color value through the object model,
	just refer to the values of the four parameters, for example \c MyG will contain the normalized green component of the color.
	...
	\param in_RedComponentParamName Script Name of the Parameter that represents the Red component of the color.
		The names of the Green, Blue and optional Alpha parameters do not need to be specified because Softimage
		uses the parameters that are consecutive after the Red component.
	...
	\sa \xt cus_ppg_RGBColorControl RGB Color Control \endxt, \xt cus_ppg_RGBAColorControl RGBA Color Control \endxt
	*/
	...
	\sa PPGLayout::PutLogic, PPGLayout::AddItem, \xt cus_ppg_CommandButton %Command Button \endxt
	*/
	...
	\sa CustomProperty::AddFCurveParameter, FCurve, PPGLayout::GetLogic, \xt cus_ppg_FCurveWidget %FCurve Widget \endxt
	*/
	...
	\note It is also still possible to read the text for a static control from a value of a string parameter, by using :::siControlStatic
		(see the ::siPPGControlType string constants).  However this function is easier and more efficient because it does not require any
	...
	\sa \xt cus_ppg_StaticText Static Text \endxt
	\since 4.2
	*/
	...
	*/
	...
	\sa PPGLayout::Delete, \xt cus_ppg_DynamicallyChangingCustomProperties Dynamically Changing Custom Properties \endxt
	*/
	CStatus Clear ();
	...
	\sa PPGLayout::PutAttribute, PPGLayout::PutLogic, PPGLayout::Clear,
		\xt cus_ppg_DynamicallyChangingCustomProperties Dynamically Changing Custom Properties \endxt
	*/
	CStatus Delete ();
	...
	This function is provided for convenience when creating the layout for an object.  Internally Softimage does not create PPGItem objects to
	represent when a row begins or ends.  Instead it uses the \a continue flag (see ::siUIContinue).

	\sa PPGLayout::EndRow, PPGItem::PutAttribute, \xt cus_ppg_WrappingControlsRows Wrapping Controls (Rows) \endxt
	*/
	CStatus AddRow ();

	/*! Completes a row that was opened by a previous call to PPGLayout::AddRow.
	\sa PPGLayout::AddRow, \xt cus_ppg_WrappingControlsRows Wrapping Controls (Rows) \endxt
	*/
	CStatus EndRow ();
	...
	\param in_ShowFrame Flag whether to draw a frame around the items in the control. When a label is specified a frame
		is always drawn (see ::siUIShowFrame).
	\param in_WidthPercentage This parameter is only applicable if the group is part of a row.  It describes how much of
		the horizontal space should be taken by this group.  If not specified, Softimage uses its own heuristic to lay out
		the group. For example, it might look better to split two groups 30%/70%, rather than using even spacing, if the
		second group has wider controls than the first.

	\sa PPGLayout::EndGroup, PPGItem::GetWidthPercentage, \xt cus_ppg_GroupingControls Grouping Controls \endxt
	*/
	...
	\sa PPGItem::AddGroup, PPGItem::GetWidthPercentage, \xt cus_ppg_GroupingControls Grouping Controls \endxt
	*/
	PPGItem EndGroup ();
	...
	\note No \c EndTab function is necessary on PPGLayout.  A tab is recognized as being complete
		when a new tab is started or the end of the layout is reached.

	\param in_Label The text that appears in the tab control.
	\sa \xt cus_ppg_UsingMultiplePagesTabs Using Multiple Pages (Tabs) \endxt
	*/
	PPGItem AddTab (const CString& in_Label);

	/*! Sets the view position, which may (but isn't guaranteed to) be used by the PPG 
	containing the layout to determine the PPG's position in screen coordinates. This is 
	currently only supported in modal PPGs.
	\param in_posX The horizontal component of the position 
	\param in_posY The vertical component of the position 
	\return OK on success; error code on failure.
	\sa PPGLayout::GetViewPosition, \xt cus_ppg_Creating_Dialogs Building Dialogs \endxt
	\since 10 (2012)
	...
	*/
	...
	/*! Returns the view position, which may (but isn't guaranteed to) be used by the PPG 
	containing the layout to determine the PPG's position in screen coordinates. This is 
	currently only supported in modal PPGs.
	\param out_posX The horizontal component of the position 
	\param out_posY The vertical component of the position 
	\return OK on success; error code on failure.
	\sa PPGLayout::SetViewPosition, \xt cus_ppg_Creating_Dialogs Building Dialogs \endxt
	\since 10 (2012)
	...
	*/
	...
	/*! Sets the view size, which may (but isn't guaranteed to) be used by the PPG 
	containing the layout to determine the PPG's size. This is currently only supported
	in modal PPGs.
	\param in_width The width of the size 
	\param in_height The height of the size 
	\return OK on success; error code on failure.
	\sa PPGLayout::GetViewSize, \xt cus_ppg_Creating_Dialogs Building Dialogs \endxt
	\since 10 (2012)
	...
	*/
	...
	/*! Returns the view size, which may (but isn't guaranteed to) be used by the PPG 
	containing the layout to determine the PPG's size. This is currently only supported 
	in modal PPGs.
	\param out_width The width of the size 
	\param out_height The height of the size 
	\return OK on success; error code on failure.
	\sa PPGLayout::SetViewSize, \xt cus_ppg_Creating_Dialogs Building Dialogs \endxt
	\since 10 (2012)
	...
	*/
	...
	/*! Returns the script code, if any, associated with the PPGLayout. This script code contains
	subroutines to react to events in the %property page. This property corresponds to the text
	found in the BeginScript/EndScript section (or equivalent Logic section) of a SPDL file.
	\return Contents of the script code associated with the Layout.
	\sa PPGLayout::PutLogic, \xt cus_ppg_DynamicallyChangingCustomProperties Dynamically Changing Custom Properties \endxt
	*/
	CString GetLogic () const;

	/*! Sets the script code associated with the PPGLayout logic.  This script code contains subroutines
	to react to events in the %property page. This property corresponds to the text found in the
	BeginScript/EndScript section (or equivalent Logic section) of a SPDL file.

	The provided string must be a correctly-formed script, written in the language specified in the
	PPGLayout::GetLanguage section.  The script is not tested for syntax errors at the time this call
	is made.

	The logic script consists of subroutines implemented in any of the supported Scripting Languages. A
	routine named "OnInit", if defined, is called when the PPG first loads.  To react to a parameter
	changing, create a routine named "PARAMSCRIPTNAME_OnChanged". To provide script code that gets
	executed when a button is pressed, create "BUTTONNAME_OnClicked".  In all these routines a global
	instance of the object PPG is available to get context information.
	...
	Rather than using this method to embed script code directly in the PPGLayout, it is also possible to
	specify an external file, using the ::siUILogicFile attribute.  See PPGLayout::PutAttribute.

	It is not necessary to set this Property for a CustomProperty that is based on a PluginItem.  In that
	case it is possible to write the event handling callbacks directly in the same script file or dll that
	implements the Plugin.

	\param in_strLogic Contents of the script code associated with the Layout.
	\sa PPGLayout::GetLogic, \xt cus_ppg_DynamicallyChangingCustomProperties Dynamically Changing Custom Properties \endxt
	*/
	...
	\return \c "JScript"
	\return \c "Python"
	\sa PPGLayout::PutLanguage, \xt cus_ppg_DynamicallyChangingCustomProperties Dynamically Changing Custom Properties \endxt
	*/
	CString GetLanguage () const;

	/*! Sets the language used in the Logic code of the %Property Page.  When not explicitly set, the default
	language matches whatever language the associated PluginItem has been implemented in. If the object in
	question is not based on a Plugin then the default language is VBScript.
	\param in_strLogicLang Name of the language to set. Possible values include \c "VBScript", \c "JScript"
		or \c "Python".
	\sa PPGLayout::GetLanguage, \xt cus_ppg_DynamicallyChangingCustomProperties Dynamically Changing Custom Properties \endxt
	*/
	CStatus PutLanguage ( const CString& in_strLogicLang );

	/*! Changes the value of an attribute of the PPGLayout.
	\param in_UIAttribute One of the \link siPPGLayoutAttribute \endlink string constants, specifying which
		attribute to set.
	\param in_AttributeValue CValue containing a new value for the attribute.  It is possible to remove an
		attribute by providing an empty CValue.
	\sa PPGLayout::GetAttribute, \xt cus_ppg_SettingControlAttributes Setting Control Attributes \endxt
	*/
	...
	This information is exposed via this function. The ::siPPGLayoutAttribute string constants provide a complete
	list of all these attributes and their meaning.

	Some of the most common attributes are also exposed directly as properties of the PPGLayout, for example
	PPGLayout::GetLogic.

	\param in_UIAttribute One of the \link siPPGLayoutAttribute \endlink string constants, specifying which
		attribute to get.
	\return Contents of the attribute.  This is empty if the attribute is not supported or has not been set.
	\sa PPGLayout::PutAttribute, \xt cus_ppg_SettingControlAttributes Setting Control Attributes \endxt
	*/
	CValue GetAttribute ( const CString& in_UIAttribute ) const;

	/*! Returns an item on the PPGLayout.  The requested name is compared with the value returned from
	PPGItem::GetName until a match is found. It is not defined which item you will get if there are multiple
	items on the layout with the same requested name.
	\param in_Name Name of the item to retrieve
	\return The specified PPGItem
	\sa \xt cus_ppg_DynamicallyChangingCustomProperties Dynamically Changing Custom Properties \endxt
	*/
	PPGItem GetItem( const CString& in_Name ) const;

	/*! Returns an item on the PPGLayout based on its index.
	\param in_Index Position of the item to retrieve
	\return The specified PPGItem
	\sa \xt cus_ppg_DynamicallyChangingCustomProperties Dynamically Changing Custom Properties \endxt
	*/
	PPGItem GetItem( LONG in_Index ) const;

	/*! Returns the number of items in the PPGLayout. This count includes all the controls, as well as special
	items like Groups and Tabs.
