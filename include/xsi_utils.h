...
public:
	...
	*/
	...
	\return True if Softimage is running on a 64-bit operating system, and False otherwise.
	*/
	static 	bool Is64BitOS();

	/*! Returns true if Softimage is running on Linux.
	\return True if Softimage is running on Linux, and False otherwise.
	*/
	static 	bool IsLinuxOS();

	/*! Returns true if Softimage is running on Microsoft Windows.
	\return True if Softimage is running on Microsoft Windows, and False otherwise.
	*/
	static 	bool IsWindowsOS();

	/*! Returns true if Softimage is running on a case-sensitive file system. For example, the file system on
	Microsoft Windows is case-insensitive, so \c Temp.txt and \c TEMP.TXT are the same file. Linux, however,
	has a case-sensitive file system. This function is provided to help write code that works properly on both
	platforms.
	...
	*/
	...
	/*! Returns the translation of a string from the specified Softimage translation dictionary. A translation
	dictionary is a table that maps English strings to another language.

	Translation dictionaries are stored in \c .dict files. The translation dictionaries for a specific language
	are stored in \c .dict files in a subfolder of \c Application/Dictionary (in the Factory location). For example,
	the Japanese translation dictionaries are stored in the \c Application/Dictionary/jp folder.

	Each string can be referenced by its numeric ID or by the English version of the string.

	Some strings contain \c "%s" placeholders for string substitutions. For example, a message like
	"File c:\temp\myfile.txt not found" is stored in the dictionary as "File %s not found".
	The \c "%s" token is a placeholder for the file name. A string can contain up to three \c %s placeholders.

	When you call Translate, the optional arguments \c in_string1, \c in_string2, and \c in_string3
	...
	*/
	...
	drive letters, server names (\c "\\server"), device names, folder names, file names, and
	special symbols such as \c ".." and \c "~".
	...
	\c BuildPath( \c "users", \c "yanick" ) returns \c "users\yanick" on Windows and
	\c "users/yanick" on Linux. The path does not need to specify an existing file or folder.

	BuildPath makes it easier to write cross-platform code that deals with file and folder paths.

	\param in_segment1 The first part of a path (for example, \c "C:" or \c "/MyDisk").
	...
	*/
	...
	/*! Makes sure that all directories in the specified path exist. For example, if the specified path is
	\c "c:\temp\a\b\c\out.txt" and \c a, \c b, and \c c do not exist, then this function creates them.

	If the path is invalid or the user does not have sufficient permissions to create the directory, this
	...
	*/
	...
	\li \b Universal: \c [Project], \c [Scene], \c [Date], \c [Time], \c [Frame], \c [Field], \c [User], \c [Host], \c [Env], and \c [Value]
	\li \b Context-Specfic: For example, the \c [Pass] tokenm which is render-specific instead of universal

	This call is only aware of the universal tokens. If any existing context-specific tokens are needed, 
	they will need to be supplied as user-defined tokens.

	To specify user-defined tokens, you can pass arrays representing token names and values
	in the \c in_TokenNames and \c in_TokenValues arguments which will be used to resolve the token
	string. The two arrays of CString values must have the same size.

	See section "Tokens and Templates" in the Softimage User Guide for a list of all the built-in tokens.

	\param in_TokenString The input string containing tokens.
	\param in_Time The time used to resolve time dependent tokens.
	\param in_bVerbose True to log error messages.
	\param in_TokenNames An optional array of strings representing the user token names.
	\param in_TokenValues An optional array of strings representing the user token values.

	\return The resolved string. In case of failure an empty CString is returned.
	\since 6.01
	*/
	static CString ResolveTokenString(
		const CString &in_TokenString,
		const CTime &in_Time,
		bool in_bVerbose,
		const CStringArray &in_TokenNames = CStringArray(),
		const CStringArray &in_TokenValues = CStringArray() );

	/*! Returns true if the file path given is an absolute path. An absolute path is a path
	that can be resolved from the root of the file system. On the Windows platform, this
	means either a path that starts with a drive letter or a full UNC path. On Linux, it
	means a path that starts with a slash (\c '/').

	\note It is not necessary for the path to exist before calling this function.

	\param in_strPath The path to test for absoluteness.
	\return true if the path is absolute.
	\since 7.0
	*/
	static bool IsAbsolutePath(
		const CString &in_strPath );
