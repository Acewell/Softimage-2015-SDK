...
/*! \class CRef xsi_ref.h
	\brief This class is used to reference Softimage objects. Each instance of an %CRef maintains a link with
	only one single object at a time, it cannot refer to multiple objects. %CRef has no base or derived class
	hierarchy and only contains methods for accessing the type of the underlying Softimage object.

	CRef provides the core of the RTTI system to determine if an object is of a specific type; for example, to
	ensure that the compatibility between the reference and a given API class is valid by checking it against
	the ::siClassID enum. %CRef instances are typically attached to API classes for manipulating Softimage objects.

	A %CRef object can be converted to its corresponding Softimage Object %Model interface, and the resulting
	object can be used with the Softimage object model. The inverse is also possible: you can create a CRef
	object from an existing Softimage object model interface. See the XSI::ConvertObject function for more details.
...
public:
	...
	*/
	...
	path is invalid.
	\tip This is the C++ API equivalent of \xl %Dictionary.GetObject
	...
	*/
	...
	/*! Less-than operator. Performs a case-sensitive comparison with a specified CRef object to determine
	if this CRef is lexicographically less than the specified CRef. The comparison is done with CRef::GetAsText.
	This is useful for sorting operations with \c stl maps.
	\param in_ref CRef we want to test.
	\return true if this CRef is less than the specified CRef, false otherwise.
	\since 7.5
	*/
	bool operator < ( const CRef& in_ref ) const;

	/*! Greater-than operator. Performs a case-sensitive comparison with a specified CRef object to determine
	if this CRef is lexicographically greater than the specified CRef. The comparison is done with CRef::GetAsText.
	\param in_ref CRef we want to test.
	\return true if this CRef is greater than the specified CRef, false otherwise.
	\since 7.5
	*/
	bool operator > ( const CRef& in_ref ) const;
...
/*! \class CRefArray xsi_ref.h
	...
	\sa CEdgeRefArray, CFacetRefArray, CParameterRefArray, CPointRefArray,
		...
		CTriangleRefArray,  CTriangleVertexRefArray, CVertexRefArray, CSIObjectRefArray
*/
//*************************************************************************
class SICPPSDKDECL CRefArray
{
	...
	*/
	 ...
	 * \return The string path list string.
	 */
	CString GetAsText() const;
...
\param in_pAuto IDispatch pointer to an %XSI Automation object model interface (see CValue::DataType)
\return The new CRef object
*/
SICPPSDKDECL CRef ConvertObject( void* in_pAuto );

/*! Converts a CRef object to an %XSI Automation (%XSI Object %Model) object.  The returned
pointer can be used to communicate with the object model.
...
SICPPSDKDECL bool HasClassID( CRef&, LONG );
SICPPSDKDECL bool CanSubClass( CRef&, LONG );
