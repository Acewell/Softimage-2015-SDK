...
/*! \class CClusterPropertyBuilder xsi_clusterpropertybuilder.h
   \brief Provides the services to accelerate the process of creating new cluster properties on a geometry.
	...
	The class saves you from creating specific clusters in order to add your properties by creating default clusters.
	These clusters are reused by the builder when adding subsequent properties. CClusterPropertyBuilder is convenient
	for importing data in Softimage where performance is critical. All operations performed by the builder are undoable
	but it is recommended to disable the undo service when using the builder for maximum results.
...
public:
	...
	*/
	...
	/*! Adds a user motion property to a default vertex cluster. The cluster used is always the size of the
	...
	*/
	...
	/*! Adds a user motion property on a specified vertex cluster. The cluster must match the size of the geometry
	...
	*/
	...
	\sa UserDataMap::SetValues, UserDataMap::SetFixedSizeValues
	*/
	CRef AddUserDataMap(
