...
/*! \class FCurve xsi_fcurve.h
	\brief The FCurve (function curve) class provides access to a set of functions for getting and setting
	Softimage fcurve attributes, as well as creating, retrieving, modifying and deleting fcurve keys.

	FCurves can be created and connected to parameters using the Parameter::AddFCurve function. FCurves are
	supported by the following parameter types: ::siBool, ::siDouble, ::siFloat, ::siInt4, ::siInt2, ::siByte,
	::siByte, ::siUInt4, ::siUInt2 and ::siUByte (see ::siVariantType for definitions of these types).

	If one method returns CStatus::AccessDenied, this may due to different reasons:
	the function curve or the function curve key(s) are locked (see the FCurve::GetLocked and FCurveKey::GetLocked methods) or
	the function curve is used by a locked animation layer (see the IsAnimationLayerLocked command).

	\note For more information on interpolation and extrapolation in Softimage, see
		\xt dev_animation_FCurveInterpolationAndExtrapolation FCurve Interpolation and Extrapolation \endxt

	\sa Parameter::GetSource, FCurveKey, \xt dev_FCurves About FCurves \endxt
...
public:
	...
	*/
	...
	\return CStatus::AccessDenied Access denied. See more information in the FCurve class description.
	...
	*/
	...
	\param in_startframe The start frame of the range of keys. Defaults to the first key on the curve.
	\param in_endframe The end frame of the range of keys. Defaults to the last key on the curve.
	\return Array of references to FCurveKey objects.
	\since 6.0
	*/
	...
	\return True if the FCurve is selected; false otherwise.
	\since 6.0
	*/
	...
	*/
	...
	\param in_value True to use SI3D tangent constraints; false for Softimage tangent constraints.
	\return CStatus::OK success
	\return CStatus::AccessDenied Access denied. See more information in the FCurve class description.
	\return CStatus::Fail failure
	*/
	...
	\return index == -1 (failure or access denied, see more information in the FCurve class description.)

	\sa FCurve::InsertKeyAtFrame
	*/
	LONG AddKey(
	...
	*/
	...
	/*! \idx %FCurve:retiming;retiming;fcurves:retiming fcurves \endidx
	...
	*/
	...
	with FCurve::BeginEdit is automatically ended before copying the %FCurve.
	...
	*/
	...
	\return -1 (failure or access denied, see more information in the FCurve class description.)
	*/
	...
	*/
	...
	/*! Returns the fcurve's mute state. If the fcurve is muted it
	will not update its corresponding parameter when the time changes.
	\return True if the fcurve is muted
	\since 6.0
	*/
	bool GetMute(void) const;

	/*! Sets the fcurve's mute state. If the fcurve is muted it
	will not update its corresponding parameter when the time changes.
	\param in_value New mute state
	\return CStatus::OK success
	\return CStatus::AccessDenied Access denied. See more information in the FCurve class description.
	\return CStatus::Fail failure
	\since 6.0
	*/
	...
	\param in_frameOffset Time offset
	\param in_valueOffset Value offset
	\return CStatus::OK success
	\return CStatus::AccessDenied Access denied. See more information in the FCurve class description.
	\return CStatus::Fail failure
	\since 6.0
	*/
	...
	\param in_Keys Keys to offset
	\param in_frameOffset Time offset
	\param in_valueOffset Value offset
	\return CStatus::OK success
	\return CStatus::Fail failure
	\return CStatus::AccessDenied Access denied. See more information in the FCurve class description.
	\return CStatus::InvalidArgument if keys are invalid or from a different fcurve
	\since 6.0
	*/
	...
	\param in_frameFactor Frame scale factor
	\param in_valueFactor Value scale factor
	\param in_frameAnchor Frame scale anchor
	\param in_valueAnchor Value scale anchor
	\return CStatus::OK success
	\return CStatus::AccessDenied Access denied. See more information in the FCurve class description.
	\return CStatus::Fail failure
	\since 6.0
	*/
	...
	\param in_Keys Keys to scale
	\param in_frameFactor Frame scale factor
	\param in_valueFactor Value scale factor
	\param in_frameAnchor Frame scale anchor
	\param in_valueAnchor Value scale anchor
	\param in_ripple True to keep keys before/after the ones being scaled the same distance from the modified keys
	\return CStatus::OK success
	\return CStatus::Fail failure
	\return CStatus::AccessDenied Access denied. See more information in the FCurve class description.
	\return CStatus::InvalidArgument if keys are invalid or from a different fcurve
	...
	*/
	...
	\param in_CyclesBefore Number of cycles to include before existing fcurve
	\param in_CyclesAfter Number of cycles to include after existing fcurve
	\return CStatus::OK success
	\return CStatus::Fail failure
	\return CStatus::AccessDenied Access denied. See more information in the FCurve class description.
	\return CStatus::InvalidArgument if a negative cycle count is specified
	\since 6.0
	*/
	...
	*/
	...
	\return CStatus::Fail If the fcurve has no keys
	\since 6.0
	*/
	...
	*/
	...
	/*! Inserts a key at the time specified by the frame argument. Unlike FCurve::AddKey,
	this member function maintains the shape of the fcurve.
	\param in_frame The frame at which to insert the new key.
	\return True success
	\return False failure or access denied, see more information in the FCurve class description
	\sa FCurve::AddKey
	\since 7.5
	*/
