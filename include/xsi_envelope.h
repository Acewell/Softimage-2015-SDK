...
/*! \class Envelope xsi_envelope.h
	...
	\eg For examples that demonstrate how to access and set deformer weights, see Envelope::GetWeights,
		Envelope::GetDeformerWeights, Envelope::SetDeformerWeights and Envelope::SetDeformerWeights2.
 */
//*****************************************************************************
...
public:
	...
	*/
	...
	\sa Envelope::GetWeights, Envelope::GetDeformerWeights, Envelope::SetDeformerWeights, Envelope::SetDeformerWeights2
	*/
	...
	\sa CClusterPropertyElementArray::GetArray, CClusterPropertyElementArray::GetValueSize,
		Envelope::GetDeformerWeights, Envelope::SetDeformerWeights, Envelope::SetDeformerWeights2
	...
	*/
	...
	\return An array of weights
	\sa Envelope::GetWeights, Envelope::GetDeformers, Envelope::SetDeformerWeights, Envelope::SetDeformerWeights2
	*/
	...
	\sa Envelope::GetDeformers, Envelope::GetWeights, Envelope::GetDeformerWeights, Envelope::SetDeformerWeights2
	...
	*/
	...
	\sa Envelope::GetDeformers, Envelope::GetWeights, Envelope::GetDeformerWeights, Envelope::SetDeformerWeights
	...
	\code
		...
		// Create the scene made of a cube and 2 nulls
		using namespace XSI;
		Application app;
		Model root = app.GetActiveSceneRoot();
		...
		Parameter param( myCube.GetKinematics().GetGlobal().GetParameters().GetItem( L"posz" ) );
		param.PutValue( (double)2.0 );
		...
		// Create a deformer array made of the 2 nulls
		CRefArray deformers( 2 );
		deformers[ 0 ] = myDeformer1;
		deformers[ 1 ] = myDeformer2;

		// Create a Envelope Weight on the grid with the 2 nulls
		Envelope myEnvelope;
		myCube.ApplyEnvelope( deformers, siUnspecified, siUnspecified, myEnvelope );

		// Get the first deformer and its weights at frame 0
		X3DObject firstDeformer( myEnvelope.GetDeformers()[ 0 ] );
		CDoubleArray weightsFirstDeformer = myEnvelope.GetDeformerWeights( firstDeformer, 0.0 );

		// Set the first deformer weights to 10 for the current frame
		// without normalizing the weights
		LONG weightCount = weightsFirstDeformer.GetCount();
		for( LONG i = 0; i < weightCount; i++ )
			weightsFirstDeformer[ i ] = 10.0;
		myEnvelope.SetDeformerWeights2( firstDeformer, weightsFirstDeformer );
		...
		// Get the second deformer
		X3DObject secondDeformer( myEnvelope.GetDeformers()[ 1 ] );

		// Get the weights of both deformers
		weightsFirstDeformer = myEnvelope.GetDeformerWeights( firstDeformer, frame.GetTime() );
		CDoubleArray weightsSecondDeformer = myEnvelope.GetDeformerWeights( secondDeformer, frame.GetTime() );

		for( LONG i = 0; i < weightCount; i ++ )
		{
			swprintf( msg, L"Vertex %i: Deformer 1: %4.1f, Deformer 2: %4.1f",
						   i, weightsFirstDeformer[ i ], weightsSecondDeformer[ i ] );
			app.LogMessage( msg );
		}
	\endcode
	*/
