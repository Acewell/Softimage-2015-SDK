 ...
 * == new in Softimage
 * ufoRasterAllocWrapper():
 *    allocate a raster object for an existing buffer 
 *    this allows calling functions that use ufoRasters with any arbitrary memory
 *    memory block
 * ufoRasterFree():
 ...
 * ufoRasterSetTitle():
 * ufoRasterGetTitle():
 * == new in Softimage.  sets a title on a newly allocated raster so that it can 
...
UFOAPI
ufoRaster ufoRasterAllocWrapper(
    ufoPixelType        pixel_type, 
    int                 x1, 
    int                 y1, 
    int                 x2, 
