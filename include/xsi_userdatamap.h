...
/*! \class UserDataMap xsi_userdatamap.h
	\brief The %UserDataMap is a Property that can be attached to any cluster.  It permits the storage
	of variable-length binary user data on the individual components of a geometry.  The data is stored
	as part of the scene file and can be accessed via plug-ins using the C++ API.

	Softimage Plug-ins (both persistent and immediate effects) can use this property to save and retrieve
	data that is associated with individual vertices, polygons, edges, or other components of an object.
	Example data possibilities include custom game structures, raw simulation data, matrices, comments and
	images.
	...
	to C++ plug-ins.  The second type of %UserDataMap uses a CustomProperty to strictly define the structure
	of the user data (see UserDataMap::GetTemplate).  Templated user data can be inspected in the UI (see the
	\xl InspectUserData command) in addition to scripting and C++ access.

	UserDataMaps are created with the \xl CreateUserDataMap command or SceneItem::AddProperty.

	%UserDataMap differs from the CustomProperty and UserDataBlob objects in that it stores separate data for
	each individual component of a cluster.

	The %UserDataMap is similar to a ClusterProperty, which also stores per-component data, but the %UserDataMap
	allows variable length data, whereas %ClusterProperty objects always store a fixed number of Doubles at each
	component.

	The User Data Map provides similar functionality to that available from the \c SAAPHIRE \c SAA_subelement*
	\c %UserData functions.  For example, the \c Name of the property corresponds to the \c Tag argument to
	functions like \c SAA_subelementSetUserData.

	It is possible to save a preset of a %UserDataMap and to reapply the user data on a different object.  However,
	...
	\code
		...
		// Copy the structure locally before erasing the data inside Softimage
...
public:
	...
	*/
	...
	/*! Returns the number of elements in the map.  This is equal to the size of the cluster that the %UserDataMap
	is attached to.
	\return Size of map.
	*/
	LONG GetCount() const ;

	/*! Flushes all data from the %UserDataMap.  This does not change the size of the %UserDataMap, but all items
	on the map will be empty.
	*/
	void Clear() ;

	/*! When the %UserDataMap is created this flag is initialized to the correct value according to the Endian
	byte order of the computer that Softimage is running on.  For example, LittleEndian is set to true on x86
	processors and false on MIPS processors.  This flag is useful when transferring Softimage scenes with binary
	data between platforms with different byte order: a plug-in can use this flag to determine whether to byte-swap
	the data retrieved from UserDataMap::GetItemValue.  Plug-ins which pack their data in a consistent order, no
	matter what platform they are on, can ignore this property.
	...
	*/
	...
	/*! A %UserDataMap can be associated with a CustomProperty that serves as a template for the data inside
	the %UserDataMap.  This allows the storage of strongly typed data on each component.  The template defines
	the format of the data, but the values of the data are stored separately for each component.

	For example, a user data map on a polygon cluster may have a template with a integer parameter \c X and a
	floating point parameter \c Y.  This means that each polygon can store its own value of \c X and \c Y.
	...
	\return Returns the template as a CustomProperty object.  If no template then an invalid CustomProperty
		object is returned.
	\sa CustomProperty
	*/
	CustomProperty	GetTemplate() const ;

	/*! Sets the template for the %UserDataMap.  To remove an existing template this can be called with an
	invalid CustomProperty object.
 	\param 	in_Template template to set
	\return CStatus
	*/
	CStatus PutTemplate( const CustomProperty& in_Template ) ;

	/*! Gets access to the data on a particular component inside the UserDataMap. The \c out_buffer will
	point to NULL if there is no user data on the specified component.
	...
	\par In the context of templated %UserDataMap:
		This function is used to transfer user data via CustomProperty::GetBinaryData and
		CustomProperty::PutBinaryData (see UserDataMap::GetTemplate). In this case you do not need to
		understand the format of the data, because you read and write the values via the ProjectItem
		functions, such as ProjectItem::GetParameters.
	...
	For performance purposes, the returned pointer points directly to internal Softimage data.  The caller
	does not need to free this memory and should treat this data as read-only and potentially transitive.
	To hold the data, it should be copied by the caller rather than holding the pointer.

	When transferring binary user data between platforms it may be necessary to perform byte-swapping.
	See UserDataMap::IsLittleEndian.
	...
	*/
	...
	On a templated %UserDataMap, you can remember the current state of the associated CustomProperty by
	calling CustomProperty::GetBinaryData and setting the returned buffer via this method.
	...
	*/
	...
	/*! Returns an array containing the user data values stored in this map.
	The number of elements in the array matches the number of geometry
	elements (\c N). The function also returns a bit array of size \c N which
	...
	*/
	...
	geometry elements in the order specified by \c in_pValues starting from
	index \c 0. Values are set directly on the property and the operation is
	not undoable.

	For large data sets, you can use an offset index to set the values in
	chunks. The offset is used for indexing into \c in_pElements which allows
	...
	\param in_pSizeArray Array of size in_nValues containing the size of each user data specified
		in \c n_pValues.
	...
	\return CStatus::InvalidArgument Returns an error if in_nElements is greater than the
		number of cluster elements or smaller than zero. Also returns an error if \c in_pElements or
		\c in_pValues are invalid.
	...
	*/
	...
	\param in_pValues Array of size \c in_nElements containing the user data values.
	\param in_pSizeArray Array of size \c in_nElements containing the size of each user data specified
		in \c in_pValues.
	\param in_nElements Number of elements in \c in_pElements.

	\return CStatus::OK Success.
	\return CStatus::InvalidArgument Returns an error if \c in_nElements is greater than the
		number of cluster elements or smaller than zero. Also returns an error if \c in_pElements or
		\c in_pValues are invalid.
	...
	*/
	...
	\c in_pValues starting from index \c 0. This version allows you to set the user data
	...
	\param in_pValues Array of size \c in_nValues containing the user data values.
	\param in_nValues Number of values in \c in_pValues.
	\param in_nSize The size of each values contained in \c in_pValues.
	\param in_nOffset Specifies the starting index in the cluster elements.

	\return CStatus::OK Success.
	\return CStatus::InvalidArgument Returns an error if \c in_nElements is greater than the
		number of cluster elements or smaller than zero. Also returns an error if \c in_pElements
		or \c in_pValues are invalid.
	...
	*/
	...
	/*! Sets the user data stored in this map with an array of values of a fixed
	size. The function sets the geometry elements in the order specified by
	\c in_ppValues starting from index \c 0. This version allows you to set the
