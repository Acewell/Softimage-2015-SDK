...
public:
	...
	*/
	...
	/*! Sets the PolygonMesh from a raw data set. If this function is used on a
	polygon mesh with some clusters and a topology change is performed, the caller
	is responsible for updating the clusters.
	...
	*/
	...
	Note: The CGeometryAccessor object can be accessed in a custom operator.
	However, the in_mode argument  is ignored if %GetGeometryAccessor is called
	on a primitive connected on the operator.
	...
	\param in_mode The construction mode is used to access a version of the geometry with
		...
		\li \c siConstructionModeDefault: Uses the current construction mode set in Softimage.
	...
	*/
	...
	/*! Returns a geometry accessor object which gives access to the mesh and
	subdivided geometry data. This is similar to GetGeometryAccessor but gives control
	over the accuracy of normal computation.
	...
	\param in_discontinuityAngle Specifies the angle of discontinuity when
		accessing the normal values on the geometry. This argument is ignored if
		in_bUseDiscontinuity is false. Defaults to 60.0.
		\param in_bQualityNormals Controls how the normals at each point are calculated
		when rendering. If this flag is true, the results is more accurate in some situations
		but takes longer to calculate. Defaults to false.
	\param in_bQualityNormals Controls how the normals at each point are calculated
		when rendering. If this flag is true, the results is more accurate in some situations
		but takes longer to calculate. Defaults to false.
	\return CGeometryAccessor object.
	\since 13.0 (2015)
	*/
	CGeometryAccessor GetGeometryAccessor2
	(
		siConstructionMode		in_mode = siConstructionModeModeling,
		siSubdivisionRuleType	in_type = siCatmullClark,
		LONG					in_subdLevel = 0,
