...
/*! \class Menu xsi_menu.h
	\brief Represents a custom menu in Softimage and provides the basic support for creating them.
	...
	\li Can be attached to Softimage native menus (aka hosting menus) sof specific views.
	...
	\li Can be shared by multiple views, the same menu can be registered in different views
		...
		of custom menus in Softimage.

	Custom menus are self-installed plug-ins and are integrated in Softimage by dropping the
	plug-in file in the %Application/Plugins folder.

	\note Softimage doesn't allow you to create a custom menu by running a script from the
		scripting editor: you must implement a menu plug-in.

	A custom menu must be registered at startup time with PluginRegistrar::RegisterMenu. The
	registration function doesn't load the custom menu explicitly in memory; Softimage loads
	the menu only when it is requested by the user.

	You must implement a callback function in order to build a menu and add menu items, this
	function is called by Softimage when the user opens the menu. The signature of this callback
	...
	\note The function return value is not required by Softimage.
...
public:
	...
	*/
	...
	/*! Add a sub-menu item to this menu onto which further MenuItem objects can be attached. This is useful for 
	grouping similar actions together when there are many options in the menu. This method is similar to Menu::AddItem
	called with siMenuItemSubmenu.
	\param in_strLabel The sub-menu item label.
	\return Newly added sub-menu object
	*/
	...
	\param in_strLabel The menu item label
	\param in_strCmd The name of a command. The API requires the command name not
	the command scripting name. Use EditCommand to find out the command name.
	...
	*/
	...
	/*! Sets the filter for this menu. Softimage uses the filter for validating the menu against the target
	or selected objects before it gets displayed. If the filter criteria are not satisfied then the custom
	menu is greyed out or simply removed if attached to a contextual menu.
	\param in_strFilter Filter name.
	\return CStatus::OK success
	\return CStatus::Fail failure
	\deprecated 6.0 This method is required for backward compatibility, use MenuItem::Filter instead.
	*/
