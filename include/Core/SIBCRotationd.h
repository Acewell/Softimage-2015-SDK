...
/*!
	\file SIBCRotationd.h
	\brief Defines the CSIBCRotationd class and the \link rotutil_group Utility functions for angles and rotations \endlink
...
/*!	Returns the angle corresponding to the given sine and cosine values.
	\param[in]	dSinus		The sine value of the desired angle.
	\param[in]	dCosinus	The cosine value of the desired angle.
	\return	The angle (in radians) corresponding to the sine and cosine values.
*/
double F3DArcSinCos( double dSinus, double dCosinus );

/*!	Limits \p io_dVal between \c in_dMin and \c in_dMax.
	\param[in,out]	io_dVal	Value to limit, receives the result of the limiting.
	\param[in]	in_dMin	The minimum value for \c io_dVal.
	\param[in]	in_dMax	The maximum value for \c io_dVal.
*/
void F3DLimit( double& io_dVal, double in_dMin, double in_dMax );

/*! Converts a quaternion into a rotation matrix.
	\param[in]	in_quat	Quaternion to convert.
	\param[out]	out_mat	Rotation matrix to receive the converted quaternion.
*/
void F3DQuatToMat( const CSIBCQuaterniond& in_quat, CSIBCRotMatd& out_mat );

/*! Converts a rotation matrix into a quaternion.
	\param[in]	in_mat		Rotation matrix to convert.
	\param[out]	out_quat	Quaternion to receive the converted rotation matrix.
*/
...
/*! Converts a set of euler angles (pitch-roll-yaw) to a quaternion.
	\param[in]	in_vct		Vector containing euler angles to convert.
	\param[out]	out_quat	Quaternion to receive the converted euler angles.
*/
...
/*! Converts a set of euler angles (pitch-roll-yaw) to a rotation matrix.
	\param[in]	in_vct		Vector containing euler angles to convert.
	\param[out]	out_mat		Rotation matrix to receive the converted euler angles.
*/
...
/*! Converts a rotation matrix into a set of euler angles (pitch-roll-yaw).
	\param[in]	in_mat		Rotation matrix to convert.
	\param[out]	out_vct		Vector to receive the euler angles for the converted rotation matrix.
*/
...
/*!	Converts a rotation matrix into a set of euler angles (pitch-roll-yaw), which
	are closest to \c inout_vct. This is provided because a single rotation matrix
	produces an infinite number of possible euler angle representations. The result
	is stored in \c in_outvct.
	\param[in]	in_mat		Rotation matrix to convert.
	\param[in,out]	inout_vct	Vector containing the euler angles that most closely represent
...
/*!	Converts a quaternion to a angle rotation about an axis.
	\param[in]	in_quat		Quaternion to convert.
	\param[out]	out_vctAxis	Vector to receive the rotation axis given by the quaternion.
	\param[out]	out_dAngle	double to receive the rotation about \p out_dAngle (in radians).
*/
...
/*!	Converts a rotation angle about a given axis to a quaternion.
	\param[in]	in_vctAxis	Axis of rotation.
	\param[in]	in_dAngle	Rotation angle (in radians) of rotation about \c in_vctAngle
	\param[out]	out_quat	Quaternion to receive the converted axis rotation.
*/
...
/*!	Returns an angle corresponding to \c in_dDesiredAngle, in the range \c [-#M_2PI, #M_2PI].
	\param[in]	in_dDesiredAngle	Angle to be bounded.
	\return	Angle corresponding to \c in_dDesiredAngle, bounded between \c [-#M_2PI, #M_2PI].
*/
double F3DBoundAngleInMinus2PITo2PI( double in_dDesiredAngle);

/*!	Converts an angle corresponding to \c in_dAngle, in the range \c [0.0f, #M_2PI].
	\param[in]	in_dAngle	Angle to be bounded.
*/
void F3DBoundAngleInZeroToTwoPI( double& in_dAngle );

/*!	Returns an angle corresponding to \c in_dAngle, in the range \c [-#M_2PI, #M_2PI]
	\param[in]	in_dAngle	Angle to be bounded.
*/
void F3DBoundAngleInMinusPIToPI( double& in_dAngle );

/*! Returns an angle corresponding to \c in_dDesiredAngle, which is as close as possible
	to \c in_dRefAngle.
	\param[in]	in_dDesiredAngle	Desired angle, bounded by \c [-#M_PI, #M_PI]
	\param[in]	in_dRefAngle		Reference angle, unbounded.
	\return	The angle corresponding to \c in_dDesiredAngle, which is as close as possible to \c in_dRefAngle
*/
double F3DGetUnboundedAngle( double in_dDesiredAngle, double in_dRefAngle );

/*!	Bounds the angles in the first column of \c io_matMN, beginning at row \c in_nFirstRow,
	and ending at row \p in_nLastRow to angles in the range [-#M_2PI, #M_2PI].
	\param[in,out]	io_matMN		Matrix containing angles in the first column to be bounded.
	\param[in]	in_nFirstRow	First row of \c io_matMN containing angles to be bound, zero-indexed
			(if negative, starts at the first row).
	\param[in]	in_nLastRow		Last row of \c io_matMN containing angles to be bound, zero-indexed
...
/*! Determines which angle \c in_dAngle is closest to: \c in_dFirstAngle; or \c in_dSecondAngle.
	\note \c in_dAngle should not be between \c in_dFirstAngle and \c in_dSecondAngle (in
		counter-clockwise order).
	\param[in]	in_dAngle		Angle for comparison.
	\param[in]	in_dFirstAngle	First angle for closeness comparison.
	\param[in]	in_dSecondAngle	Second angle for closeness comparison.
	\return	\c in_dFirstAngle or \c in_dSecondAngle, whichever is closest to \c in_dAngle.
*/
...
/*!	\brief Class representing a generic rotation object.
	...
	public:
		...
		*/
		...
		/*! Constructs a rotation object representing the rotation given by the quaternion
			\p in_quat.
			\param[in]	in_quat	Quaternion representing the rotation for the new rotation object.
		*/
		...
		/*! Constructs a rotation object representing the rotation given by the rotation
			matrix \p in_matRot.
			\param[in]	in_matRot	Rotation matrix representing the rotation for the new rotation object.
		*/
		...
		/*! Constructs a rotation object representing the rotation given by the set of euler angles
			...
			\param[in]	in_vctXYZAngles	Set of euler angles representing the rotation for the new rotation
					object.
		*/
		...
		/*! Constructs a rotation object representing the rotation given by the set of euler angles
			...
			\param[in]	in_dX	The euler angle containing the pitch angle (in radians).
			\param[in]	in_dY	The euler angle containing the roll angle (in radians).
			\param[in]	in_dZ	The euler angle containing the yaw angle (in radians).
		*/
		...
		/*! Constructs a rotation object representing the rotation given by the rotation of \p in_dAngle
			...
			\param[in]	in_axis		Major axis of rotation.
			\param[in]	in_dAngle	Angle of rotation about the given major axis (in radians).
		*/
		...
		/*! Constructs a rotation object representing the rotation given by the rotation of \p in_dAngle
			...
			\param[in]	in_vctAxis	Axis of rotation.
			\param[in]	in_dAngle	Angle of rotation about the given axis (in radians).
		*/
		...
		/*!	Constructs a rotation object representing the rotation required to rotated \p in_vctFrom to
			\p in_vctTo.
			\param[in]	in_vctFrom	Starting point of rotation.
			\param[in]	in_vctTo	Ending point of rotation.
		...
		*/
		...
		/*! Sets this rotation to the identity rotation (no rotation).
			\return	Reference to this rotation object.
		...
		*/
		...
		/*! Determines whether this rotation is the identity rotation (no rotation).
			\note This function uses no floating-point error range, meaning that the rotation
				object must be exactly the identity rotation.
			\retval	true if the rotation object is the identity rotation
		...
		*/
		...
		/*! Determines whether this rotation is the identity rotation (no rotation). Since this
			...
			\param[in]	in_rdTolerance	Floating-point comparison error range. Defaults to #PICO_EPS.
			\retval	true if the rotation object is the identity rotation
			\retval	false otherwise
			\sa CSIBCRotationd::IsIdentity
			\sa CSIBCRotationd::SetIdentity
		*/
		...
		/*!	Returns the quaternion representation of this rotation object.
			\param[out]	out_quat	Receives the quaternion representation of this object.
		...
		*/
		...
		/*!	Returns the rotation matrix representation of this rotation object.
			\note The return value of this function should not be freed.
			\return	Pointer to the rotation matrix representation of this rotation object.
		...
		*/
		...
		/*!	Returns the rotation matrix representation of this rotation object.
			\param[out]	out_matRot	Receives the rotation matrix representation of this rotation object.
		...
		*/
		...
		/*! Returns the euler angles representation of this rotation object (pitch-roll-yaw, in radians).
			\param[out]	out_vctXYZAngles	Receives the euler angles representation of this rotation object.
		...
		*/
		...
		/*! Returns the euler angles representation of this rotation object (pitch-roll-yaw in radians).
			\param[out]	out_dX	Receives the pitch euler angle representation of this rotation object.
			\param[out]	out_dY	Receives the roll euler angle representation of this rotation object.
			\param[out]	out_dZ	Receives the yaw euler angle representation of this rotation object.
			\sa CSIBCRotationd::GetXYZAngles
			\sa CSIBCRotationd::GetQuat
			\sa CSIBCRotationd::GetMatrix
			\sa CSIBCRotationd::GetAxisAngles
		*/
		...
		/*!	Returns the axis rotation representation of this rotation object.
			\param[out]	out_vctAxis	Receives the arbitrary axis of rotation of this rotation object.
			\param[out]	out_dAngle	Receives the angle of rotation (in radians) about the axis of rotation for this rotation object.
			\sa CSIBCRotationd::GetXYZAngles
			\sa CSIBCRotationd::GetQuat
			\sa CSIBCRotationd::GetMatrix
		*/
		...
		/*!	Sets this rotation object from another rotation object \p in_rot.
			\param[in]	in_rot	Rotation object to set this object equal to.
			\return	Reference to this rotation object.
		...
		*/
		...
		/*!	Sets this rotation object from a quaternionion, \p in_quat.
			\param[in]	in_quat	Quaternion representing the rotation to set this object equal to.
			\return	Reference to this rotation object.
			\sa CSIBCRotationd::Set
		*/
		...
		/*!	Sets this rotation object from a rotation matrix, \p in_matRot.
			\param[in]	in_matRot	Rotation matrix representing the rotation to set this object equal to.
			\return	Reference to this rotation object.
			\sa CSIBCRotationd::Set
		*/
		CSIBCRotationd& Set( const CSIBCRotMatd& in_matRot );

		/*!	Sets this rotation object from a set of euler angles (pitch-roll-yaw, in radians), \p in_vctXYZAngles.
			\param[in]	in_vctXYZAngles	Sets of euler angles representing the rotation to set this object equal to.
			\return	Reference to this rotation object.
			\sa CSIBCRotationd::Set
		*/
		CSIBCRotationd& Set( const CSIBCVector3Dd& in_vctXYZAngles );

		/*!	Sets this rotation object from a set of euler angles (pitch-roll-yaw, in radians), \p in_dX, \p in_dY and \p in_dZ.
			\param[in]	in_dX	Pitch euler angle representing the rotation to set this object equal to.
			\param[in]	in_dY	Roll euler angle representing the rotation to set this object equal to.
			\param[in]	in_dZ	Yaw euler angle representing the rotation to set this object equal to.
			\return	Reference to this rotation object.
			\sa CSIBCRotationd::Set
		*/
		CSIBCRotationd& Set( double in_dX, double in_dY, double in_dZ );

		/*! Sets this rotation object from a rotation angle \p in_dAngle about a major axis given by \p in_axis.
			\param[in]	in_axis		Axis of rotation.
			\param[in]	in_dAngle	Angle of rotation about \p in_axis to set this rotation object equal to.
			\return	Reference to this rotation object.
			\sa CSIBCRotationd::Set
		*/
		CSIBCRotationd& Set( const E3DAxisType in_axis, const double in_dAngle );

		/*!	Sets this rotation object from a rotation angle \p in_dAngle about an arbitrary axis given by \p in_vctAxis.
			\param[in]	in_vctAxis	Axis of rotation.
			\param[in]	in_dAngle	Angle of rotation about \p in_vctAxis to set this rotation object equal to.
			\return	Reference to this rotation object.
			\sa CSIBCRotationd::Set
		*/
		...
		*/
		...
		/*!	Sets this rotation object to the rotation defined by the X, Y and Z axis of a rotation
			...
			\param[in]	in_vctX		X-axis for the rotation to set this rotation object equal to.
			\param[in]	in_vctY		Y-axis for the rotation to set this rotation object equal to.
			\param[in]	in_vctZ		Z-axis for the rotation to set this rotation object equal to.
			\return	Reference to this rotation object.
		*/
		...
		/*! Sets this rotation object to align with a given X world axis, using
			...
			\param[in]	in_vct		X-axis for the world vector to align with.
			\param[out]	out_bFlip	Set to true if the current rotation object is diametrically
					opposed to \p in_vct, and indicates that a sudden flip in the rotation
					object has occurred, false otherwise.
			\retval	true if the alignment was successful
			\retval	false if the input vector \p in_vct is the null vector.
		...
		*/
		...
		/*! Sets this rotation object to align with a given Y world axis, using
			the minimal angular displacement.
			\param[in]	in_vct		Y-axis for the world vector to align with.
			\param[out]	out_bFlip	Set to true if the current rotation object is diametrically
				opposed to \p in_vct, and indicates that a sudden flip in the rotation
				object has occurred, false otherwise.
			\retval	true if the alignment was successful
		...
		*/
		...
		/*! Sets this rotation object to align with a given Z world axis, using
			the minimal angular displacement.
			\param[in]	in_vct		Z-axis for the world vector to align with.
			\param[out]	out_bFlip	Set to true if the current rotation object is diametrically
				opposed to \p in_vct, and indicates that a sudden flip in the rotation
				object has occurred, false otherwise.
			\retval	true if the alignment was successful
		...
		*/
		...
		/*! Aligns this rotation object first with the major axis given by \p in_eFirstAxis,
			...
			\param[in]	in_eFirstAxis	The major axis of the first alignment.
			\param[in]	in_vct1			The world vector for the major axis defined by \p in_eFirstAxis.
			\param[in]	in_eSecondAxis	The major axis of the secodn alignment.
			\param[in]	in_vct2			The world vector for the major axis defined by \p in_eSecondAxis.
			\param[out]	out_bFlip		Set to true if the current rotation object is diametrically
			...
			\retval true if the alignment was successful
			\retval false if the alignment axes in \p in_eFirstAxis and \p in_eSecondAxis are the same.
			\sa CSIBCRotationd::AlignToX
			\sa CSIBCRotationd::AlignToY
			\sa CSIBCRotationd::AlignToZ
		*/
		...
		/*!	Computes the multiplication of \p in_rot1 and \p in_rot2 (<TT>in_rot1 * in_rot2</TT>),
			and stores the result in this rotation object.
			\param[in]	in_rot1	First rotation object in the multiplication (on the left).
			\param[in]	in_rot2	Second rotation object in the multiplication (on the right).
			\return	Reference to this rotation object.
		...
		*/
		...
		/*!	Computes the multiplication of this rotation object and \p in_rot (<TT>this * in_rot</TT>),
			and stores the result in this rotation object.
			\param[in]	in_rot	Rotation object in the multiplication (on the right).
			\return	Reference to this rotation object.
			\sa	CSIBCRotationd::Mul
			\sa	CSIBCRotationd::LeftMul
			\sa CSIBCRotationd::MulUsingEulerAnglesTarget
		...
		*/
		...
		/*!	Computes the multiplication of \p in_rot and this rotation object (<TT>in_rot * this</TT>),
			and stores the result in this rotation object.
			\param[in]	in_rot	Rotation object in the multiplication (on the left).
			\return	Reference to this rotation object.
			\sa	CSIBCRotationd::Mul
			\sa	CSIBCRotationd::LeftMul
			\sa CSIBCRotationd::MulUsingEulerAnglesTarget
		...
		*/
		...
		/*! Computes the multiplication of \p in_rot1 and \p in_rot2 (<TT>in_rot1 * in_rot2</TT>),
			...
			\param[in]	in_rot1	First rotation object in the multiplication (on the left).
			\param[in]	in_rot2	Second rotation object in the multiplication (on the right).
			\param[in]	in_oEulerAngles	Euler angles most closely representing the desired
		...
		*/
		...
		/*!	Computes the multiplication of \p in_rot1 and \p in_rot2 (<TT>in_rot1 * in_rot2</TT>),
			...
			\param[in]	in_rot1	First rotation object in the multiplication (on the left).
			\param[in]	in_rot2	Second rotation object in the multiplication (on the right).
			\param[in]	in_ulUseRot1OrUseRot2AsTarget	If 1, uses \p in_rot1 as the euler angle target,
		...
		*/
		...
		/*!	Sets this rotation object to the inverse of \p in_rot.
			\param[in]	in_rot	Rotation object containing the inverse to set as this rotation object.
			\return	Reference to this rotation object.
		...
		*/
		...
		/*!	Sets this rotation object to the interpolation between the rotation objects
			...
			\param[in]	in_rot1	Starting point for rotation interpolation.
			\param[in]	in_rot2	Ending point for rotation interpolation.
			\param[in]	in_dU	Interpolation between \p in_rot1 and \p in_rot2, in the range <TT>[0.0f, 1.0f]</TT>.
			\return	Reference to this rotation object.
		*/
		...
		/*!	Applies this rotation object to a single vector.
			\param[in]	in_vct	The vector to be rotated.
			\param[out]	out_vct	Receives the rotated vector.
			\return	Reference to \p out_vct.
		...
		*/
		...
		/*!	Applies this rotation object to multiple vectors.
			\param[in]	in_vcts		Array of vectors.
			\param[in]	in_lNbVcts	Number of vectors in \p in_vcts (and \p out_vcts).
			\param[out]	out_vcts	Array of vectors receiving the array rotated vectors.
			\sa CSIBCRotationd::RotateVct
			\sa CSIBCRotationd::InvRotateVct
		*/
		...
		*/
		...
		/*!	Maps a direction vector \p in_vctDir from an orientation space, given by
			...
			\param[in]	in_rotFrom	Rotation object representing the orientation space which
				\p in_vctDir is defined in.
			\param[in]	in_vctDir	The direction vector, defined in \p in_rotFrom.
			\param[out]	out_vctDir	Receives \p in_vctDir mapped from \p in_rotFrom to world
				orientation space.
			return	Reference to \p out_vctDir
		...
		*/
		...
		/*!	Maps a direction vector \p in_vctDir from world orientation space, to an
			...
			\param[in]	in_rotTo	The orientation space to map \p in_vctDir to.
			\param[in]	in_vctDir	The direction vector, defined in world orientation space.
			\param[out]	out_vctDir	Receives \p in_vctDir mapped from world	orientation space to \p in_rotFrom.
			return	Reference to \p out_vctDir
		...
		*/
		...
		/*!	Maps a direction vector \p in_vctDir from one orientation space \p in_rotFrom,
			...
			\param[in]	in_rotTo	The orientation space to map \p in_vctDir to.
			\param[in]	in_vctDir	The direction vector, defined in world orientation space.
			\param[out]	out_vctDir	Receives \p in_vctDir mapped from world	orientation space
				to \p in_rotFrom.
			return	Reference to \p out_vctDir
			\sa CSIBCRotationd::MapDirSpaceToWorld
			\sa CSIBCRotationd::MapDirWorldToSpace
		*/
		...
		/*! Not implemented yet*/
		friend bool AreAlmostEqual( const CSIBCRotationd& in_rot1,/*!< n/a */
								  const CSIBCRotationd& in_rot2,/*!< n/a */
								  const double in_dEpsilon = PICO_EPS /*!< n/a */);

		// operator==			| Verify equality between rotations.
		/*! Not implemented yet */
		bool operator ==( const CSIBCRotationd& in_rot /*!< n/a */) const;
