...
/*! \class X3DObject xsi_x3dobject.h
	\brief The X3DObject object represents a basic 3D object of a scene in Softimage.
...
public:
	...
	*/
	...
	\note Although the scene root is a kind of Model it does not support primitives.
	...
	*/
	...
	\param in_polygonData	An ordered array of polygon definitions, each polygon is defined by a list of
		...
		polygon. The ordering of vertices must respect a counter-clockwise ordering to get out going normals
		(right-hand rule). For example, array of polygons with 4 indices each: \c {4,0,1,4,3,4,1,2,5,4... }
	\param in_name			Name of child.
	\param io_3dobj			New child object.
	\return CStatus::OK success
	\return CStatus::Fail failure
	...
	*/
	...
	\param in_preset Preset name member of the Camera family (%Camera, Telephoto, Wide_Angle, etc.)
	...
	*/
	...
	/*! Finds the first X3DObject child object of an X3DObject object that matches a set of search criteria. The search
	is done recursively by default, the function returns the object that meet all supplied criteria such as name, type
	and family(ies). This function is typically used for retrieving an object if you know its type or name for instance.

	\deprecated 9.0 (2011) Use X3DObject::FindChild2 instead.
	...
	\param in_type The type of object defined by ::siType or an empty string if no type is used. The type can be a
		...
		or siSrfMeshPrimType. If a primitive type is supplied, the function considered the children objects defined
		with a primitive of this type. The types must be one of the following:

		\code
						si3DObjectType							siDodecahedronPrimType					siSpherePrimType
						siArcPrimType							siDragCtrlPrimType						siSpiralPrimType
						siAttractorCtrlType						siEddyCtrlPrimType						siSpotInterestPrimType
						siCameraPrimType						siFanType								siSpotRootPrimType
						siCameraRootPrimType					siFurPrimType							siSquarePrimType
						siChainBonePrimType						siGeoShaderPrimType						siSrfMeshPrimType
						siChainEffPrimType						siGravityCtrlType						siStandInPrimType
						siChainRootPrimType						siGridPrimType							siTetrahedronPrimType
						siCirclePrimType						siIcosahedronPrimType					siTorusPrimType
						siCloudPrimType							siLatticePrimType						siTurbulenceCtrlPrimType
						siConePrimType							siLightPrimType							siVolumeDeformType
						siCrvListAggregatePrimType				siModelNullPrimType						siVortexCtrlType
						siCrvListPrimType						siModelType								siWaveCtrlType
						siCubePrimType							siNullPrimType							siWindType
						siCylinderPrimType						siOctahedronPrimType
	...
	\return X3DObject The object that matches the search criteria or empty object otherwise.
	...
	*/
	...
	is done recursively, the function returns all objects satisfying the following criteria: name, type and family(ies).

	\deprecated 8.0 (2010) Use X3DObject::FindChildren2 instead.
	...
	*/
	...
	\warning UV ordering is different for the C++ API than it is for scripting. In scripting, a
		...
		\c in_nurbsFormat argument. See \xt cpp_understand_UV_Ordering UV Ordering \endxt for a sample conversion function.
	...
	*/
	...
	to call the deprecated \a CreateParticleCloud command and then freeze the result.	Any particle type
	that will be specified in calls to ParticleCloudPrimitive::AddParticles should be added initially to
	the cloud by specifying it in the \c in_parTypes argument.
	...
	\return CStatus::OK success
	\return CStatus::Fail failure
	\deprecated 8.0 (2010) Legacy particles are only kept for backward compatibility.
	*/
	...
	/*! Returns the local scaling. This is a shortcut. You can also retrieve the local scaling by
	...
	*/
	...
	/*! Sets the local translation. This is a shortcut. You can also set the local scaling by calling
	X3DObject::GetKinematics, Kinematics::GetLocal, KinematicState::PutTransform.
	...
	*/
	...
	/*! Returns a rigid body accessor object which gives access to the rigid body simulation
	parameters and connections.
	\return CRigidBodyAccessor object.
	\since 6.0
	*/
	CRigidBodyAccessor GetRigidBodyAccessor( ) const;

	/*! Returns a rigid constraint accessor object which gives access to the rigid body simulation
	parameters and connections.
	\return CRigidConstraintAccessor object.
	\since 6.0
	*/
	CRigidConstraintAccessor GetRigidConstraintAccessor( ) const;

	/*! Calculates and returns the minimum and maximum coordinates for
    the visible bounding box of the X3DObject. The input transformation will is applied
	to the bounding box when it is calculated. If the input transformation is an identity
	transform then the bounding box that is returned will be in the object's local coordinate
	space.
	<br>
	If the bounding box is undefined then CStatus::False will be returned and the
	minimum coordinates will be greater than the maximum coordinates. This could happen
	if the object is hidden or if the object does not contain any geometry.
	Note: Only one coordinate needs to be tested to determine if the bounding box is
	undefined. (out_minX > out_maxX)
	\retval out_minX  Returns the minimum x coordinate of the bounding box.
	\retval out_minY  Returns the minimum y coordinate of the bounding box.
	\retval out_minZ  Returns the minimum z coordinate of the bounding box.
	\retval out_maxX  Returns the maximum x coordinate of the bounding box.
	\retval out_maxY  Returns the maximum y coordinate of the bounding box.
	\retval out_maxZ  Returns the maximum z coordinate of the bounding box.
	\param in_XfoObjectToBBoxSpace  Contains a transform (if desired) between the object and the global coordinate axes.
	\param in_bRecursive  If true include children, otherwise return bounding box for input X3DObject only. (default)
	\return CStatus::OK success
	\return CStatus::False bounding box is undefined
	\return CStatus::Fail failure
	\since 10.0 (2012)
	*/
	...
	\param in_text Text annotation to add.
	\param in_name <Name of the new <object idref="Annotation"/>
